<!DOCTYPE html>
<html>
<head>
        <meta charset="UTF-8">
	
	<meta property="og:title" content="Baltic Center - Шетелде оқу, арман ба әлде шындық па?" />
        <meta property="og:description" content="Білім алу – Сіздің болашағыңызға деген ұзақ мерзімді инвестиция, сондықтан университет таңдау мәселесіне аса маңызды түрде қарау керек. Осы кезеңде барлық адамдар шетелде білім алуға құлшыныс білдіреді және ол бекер емес!" /> 
        <meta property="og:image" content="http://baltic-center.eu/img/header_background.jpg" />
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
    <meta name="description" content="Описание">
    <meta name="keywords" content="Описание">
    <meta name="google-site-verification" content="S386zaI2q9nFsR9pmcPC1nt3toyeSqg1xFdgyrWleA8" />
    <meta name="google-site-verification" content="xHMOb3cAs40Fa2r6LM5jKQyKLoZRRVaXkiSIKVDUoBk" />

    <title>Baltic Center - Шетелде оқу, арман ба әлде шындық па?</title>




    <link rel="stylesheet" href="../css/bootstrap.css">
    <link rel="stylesheet" href="../css/font-awesome.min.css">
    <link rel="stylesheet" href="../css/hover.css">


    <link rel="stylesheet" href="../css/style.css">

    <link rel="icon" type="image/png" href="../favicon.png" />
    <link rel="apple-touch-icon" href="../apple-touch-favicon.png"/>
    <script src="../js/jquery-1.12.4.min.js"></script>

    <script>
     (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
      (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
      m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
      })(window,document,'script','//www.google-analytics.com/analytics.js','ga');

      ga('create', 'UA-76055340-1', 'auto');
      ga('send', 'pageview');
    </script>
</head>
<body>

<!-- Yandex.Metrika counter -->
<script type="text/javascript">
    (function (d, w, c) {
        (w[c] = w[c] || []).push(function() {
            try {
                w.yaCounter36592180 = new Ya.Metrika({
                    id:36592180,
                    clickmap:true,
                    trackLinks:true,
                    accurateTrackBounce:true,
                    webvisor:true
                });
            } catch(e) { }
        });

        var n = d.getElementsByTagName("script")[0],
            s = d.createElement("script"),
            f = function () { n.parentNode.insertBefore(s, n); };
        s.type = "text/javascript";
        s.async = true;
        s.src = "https://mc.yandex.ru/metrika/watch.js";

        if (w.opera == "[object Opera]") {
            d.addEventListener("DOMContentLoaded", f, false);
        } else { f(); }
    })(document, window, "yandex_metrika_callbacks");
</script>
<noscript><div><img src="https://mc.yandex.ru/watch/36592180" style="position:absolute; left:-9999px;" alt="" /></div></noscript>
<!-- /Yandex.Metrika counter -->

<?php require 'blocks/header.php'; ?>
<div class="contaner-fluid additional-color">
<!--  Breadcrumbs-->
 <!-- <div class="container hlebnie-kroshki">
     <div class="row">
         <ol class="breadcrumb">
      <li><a href="index.php">Главная</a></li>

      <li class="active">Контакты</li>
</ol>
     </div>
 </div> -->

<!--About    -->


    <section>
        <div class="container contacts">
            <div class="row">
               <h1>Байланыстар</h1>
                <div class="col-md-6">
                    <h2>Латвиядағы бас кеңсе:</h2>
                    <span class="bold">Жұмыс режимі: </span>  <br>
                    Дүйсенбі - Жұма: с 10-00 до 19-00 <br>
                    Сенбі - Жексенбі: Демалыс  <br><br>

                    <span class="bold">Мекен жайы:</span> Marupesiela 2, Riga, LV-1002, Latvia<br><br>

                    <span class="bold">Телефондары:</span> +371 24332827, +371 22169545 +371 67398609<br><br>

                    <span class="bold">E-mail:</span> admission@baltic-center.eu, info@baltic-center.eu<br><br>

                    <span class="bold">Skype:</span> baltic-center.eu
                </div>

                <div class="col-md-6">
                    <h2>Өзбекстанда:</h2>
                    <span class="bold">Жұмыс режимі: </span>  <br>
                    Дүйсенбі - Жұма: с 10-00 до 19-00 <br>
                    Сенбі - Жексенбі: Демалыс  <br><br>

                    <span class="bold">Мекен жайы:</span> Өзбекстан, 100043, Ташкент, Бунедкор даңғылы, 15<br><br>

                    <span class="bold">Телефондары:</span> +99890 9431158, +99871 2882641<br><br>

                    <span class="bold">E-mail:</span> uzbekistan@baltic-center.eu<br><br>

                    <span class="bold">Skype:</span> baltic-center.eu
                </div>
            </div>

            <div class="row margin-top-2">
                <div class="col-md-6">
                    <h2>Қазақстанда:</h2>
                    <span class="bold">Жұмыс режимі: </span>  <br>
                    Дүйсенбі - Жұма: с 10-00 до 19-00 <br>
                    Сенбі - Жексенбі: Демалыс  <br><br>

                    <span class="bold">Мекен жайы:</span> Қазақстан, 050012, Алматы, Мәуленов көш., 85-үй, 3-қабат, 50-кеңсе<br><br>

                    <span class="bold">Телефондары:</span> +7727 2676272, +7778 3676202, +7778 3679234<br><br>

                    <span class="bold">E-mail:</span> kazakhstan@baltic-center.eu<br><br>

                    <span class="bold">Skype:</span> baltic-center.kz
                </div>
                <div class="col-md-6">
                    <h2>Тәжікстанда:</h2>
                    <span class="bold">Жұмыс режимі: </span>  <br>
                    Дүйсенбі - Жұма: с 10-00 до 19-00 <br>
                    Сенбі - Жексенбі: Демалыс  <br><br>

                    <span class="bold">Мекен жайы:</span> 734003, Душанбе, Рудаки даңғылы, 127, "Рудаки Плаза" бизнес орталығы, 507-кеңсе<br><br>

                    <span class="bold">Телефондары:</span> +99293 5709201, +992 935001639<br><br>

                    <span class="bold">E-mail:</span> tajikistan@baltic-center.eu <br> admission@baltic-center.eu<br><br>

                    <span class="bold">Skype:</span> baltic.center
                </div>
            </div>
            <div class="hr"></div>

            <div class="row margin-top-2">
               <span class="tipa-h1">Біздің серіктестер</span>
                <div class="col-md-6">
                    <h2>Украина:</h2>
                    <span class="bold">Компаниясы:</span>
                    Verainfo <br><br>


<!--                    <span class="bold">Мекен жайы:</span> ул.Мауленова, дом 85, 3 этаж, офис 50, Алматы, Казахстан<br><br>-->

<!--                    <span class="bold">Телефондары:</span> +7727 2676272, +7778 3676202, +7778 3679234<br><br>-->

                    <span class="bold">E-mail:</span> verainfo@ukr.net<br><br>

                    <span class="bold">Сайты:</span> <a href="http://virainfo.kiev.ua/" target="_blank">virainfo.kiev.ua</a>
                </div>

                <div class="col-md-6">
                    <h2>Өзбекстан:</h2>
                    <span class="bold">Компаниясы:</span>
                    Alfakom Training Center<br><br>


                    <span class="bold">Мекен жайы:</span> Мустакиллик даңғылы, 68, "Узавтойул" ғимараты<br><br>

                    <span class="bold">Телефондары:</span> 202-25-75 (+99898) 366-25-75<br><br>

                    <!--<span class="bold">E-mail:</span> verainfo@ukr.net<br><br>-->

                    <span class="bold">Сайты:</span> <a href="http://www.alfakom.uz" target="_blank">alfakom.uz</a>
                </div>


            </div>


        </div>
    </section>





 </div>


  <?php require 'blocks/footer.php'; ?>






        <script src="../js/bootstrap.min.js"></script>



<!--BEGIN JIVOSITE CODE {literal} -->
<script type='text/javascript'>
(function(){ var widget_id = '9B8okHwNjb';var d=document;var w=window;function l(){
var s = document.createElement('script'); s.type = 'text/javascript'; s.async = true; s.src = '//code.jivosite.com/script/widget/'+widget_id; var ss = document.getElementsByTagName('script')[0]; ss.parentNode.insertBefore(s, ss);}if(d.readyState=='complete'){l();}else{if(w.attachEvent){w.attachEvent('onload',l);}else{w.addEventListener('load',l,false);}}})();</script>
<!-- {/literal} END JIVOSITE CODE -->

</body>
</html>
